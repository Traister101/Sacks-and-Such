...
public final class RecipeRegistry {
	...
	public static void onRegisterKnappingRecipeEvent(Register<KnappingRecipe> event) {
		...
		if (ConfigSNS.SACK.LEATHER_SACK.DANGEROUS.isEnabled)
...
//				" XXX ");
	...
	public static void onRegisterLoomRecipeEvent(Register<LoomRecipe> event) {
		...
		recipeRegistry.register(
				new LoomRecipe(name, IIngredient.of(ItemsSNS.REINFORCED_FIBER, 16), new ItemStack(ItemsSNS.REINFORCED_FABRIC), 16, texture));
	}

	private static void heatRecipe(IForgeRegistry<HeatRecipe> recipeRegistry, @Nonnull Item itemInput, @Nonnull Item itemOutput) {
		recipeRegistry.register(new HeatRecipeSimple(IIngredient.of(itemInput), new ItemStack(itemOutput), 1599f, Metal.Tier.TIER_I).setRegistryName(
				itemInput.getRegistryName()));
	}
	...
	private static void leatherKnappingRecipe(IForgeRegistry<KnappingRecipe> recipeRegistry, @Nonnull Item item, int count, String... pattern) {
		recipeRegistry.register(
				new KnappingRecipeSimple(KnappingType.LEATHER, true, new ItemStack(item, count), pattern).setRegistryName(item.getRegistryName()));
	}
	...
	private static void clayKnappingRecipe(IForgeRegistry<KnappingRecipe> recipeRegistry, @Nonnull Item item, int count, String... pattern) {
		recipeRegistry.register(
				new KnappingRecipeSimple(KnappingType.CLAY, true, new ItemStack(item, count), pattern).setRegistryName(item.getRegistryName()));
	}
	...
	private static void fireClayKnappingRecipe(IForgeRegistry<KnappingRecipe> recipeRegistry, @Nonnull Item item, int count, String... pattern) {
		recipeRegistry.register(
				new KnappingRecipeSimple(KnappingType.FIRE_CLAY, true, new ItemStack(item, count), pattern).setRegistryName(item.getRegistryName()));
	}
}